...
function fOnDrag(y,m,d,aStat,e) {
	if (aStat==0) {
		if (!(OP||IE&&MAC)) fRunInOtherCtx("__lastDrag=true");	// set the drag flag to enable dragging to other calendar panel
		var bRangeClick=e?fCheckKeyPressed(1+2+4,e):false;	// use ctrl, shift or alt to select range end
		fAddRange(y,m,d,bRangeClick);
		fRunInOtherCtx("fRepaint()");
		fRepaint();
		if (gContainer.fOnRange) gContainer.fOnRange(gc._cxp_pds);
	} else if (aStat==2) fRunInOtherCtx("__lastDrag=null");
	else { // aStat==1
		fAddRange(y,m,d,true);
		if (gContainer.fOnRange) gContainer.fOnRange(gc._cxp_pds);	// pass in selected range
		fRunInOtherCtx("fRepaint()");
		fRepaint();
	}
...
function fIsSelected(y,m,d) {
	if (gc._cxp_pds.length==0) return false;
	var dt=Date.UTC(y,m-1,d);
	return (gc._cxp_pds[0]<=dt&&dt<=gc._cxp_pds[1]);
...
// make the 2nd calendar 1 month later than current.
...
var gc=gContainer;	// make gc._cxp_pds a shared object between calendar instances.
gc._cxp_pds=[]; // will contain 3 items: [startOfSelected, endOfSelected, firstClicked].

function fRunInOtherCtx(cmd) {
	var thisCtx=gTheme[3].split("_");
	for (var i=1;i<=_calendars4sync;i++)
		if (thisCtx[1]!=i)
			eval("gContainer."+thisCtx[0]+"_"+i+"."+cmd);
}
...
function fClearAll() {
	gc._cxp_pds.length=0;
}

function fAddRange(y,m,d,bBatch) {
	var dt=Date.UTC(y,m-1,d);
	if (bBatch&&gc._cxp_pds.length>0) {
		if (dt<gc._cxp_pds[2]) {gc._cxp_pds[0]=dt; gc._cxp_pds[1]=gc._cxp_pds[2];}
		else if (gc._cxp_pds[2]<dt) {gc._cxp_pds[0]=gc._cxp_pds[2]; gc._cxp_pds[1]=dt;}
	} else gc._cxp_pds[0]=gc._cxp_pds[1]=gc._cxp_pds[2]=dt;
}
