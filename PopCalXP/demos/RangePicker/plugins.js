...
function fOnDrag(y,m,d,aStat,e) {
	if (aStat==0) {
		...
		if (gContainer.fOnRange) gContainer.fOnRange(_pds);
		_showRange=bRangeClick;
		fUpdSelect(y,m,d);
		if (!(IE&&MAC)) fRepaint();
	} else if (aStat==1) {
		var ag=fGetAgenda(y,m,d,true);
		if (ag==null||ag[1]==null) return;
...
function fIsSelected(y,m,d) {
	...
	if (_showRange) return _pds[0]<=dt&&dt<=_pds[1];
	else return gdSelect[2]==d&&gdSelect[1]==m&&gdSelect[0]==y;
}
...
// stuff in new bottom section
gsBottom="<A href='javascript:void(0)' class='BottomAnchor' onclick='if(this.blur)this.blur();fHideCal();return false;' onmouseover='return true;'>close the calendar</A>";
gbAutoClose=false;	// override theme option to disable auto-close
