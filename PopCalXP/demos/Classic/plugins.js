...
// posc is the popup locator tag - where to popup
var _startc,_endc;
function fStartPop(startc,endc, posc) {
  _startc=startc;
  _endc=endc;
  var sd=fParseInput(endc.value); 
  if (!sd) sd=gEnd;
  fPopCalendar(startc, [gBegin,sd,sd], null, posc);
}

function fEndPop(startc,endc, posc) {
  _startc=startc;
  _endc=endc;
  var sd=fParseInput(startc.value);
  if (!sd) sd=gBegin; 
  fPopCalendar(endc, [sd,gEnd,sd], null, posc);
}
...
function fClearSelected() { // clear the date field and reset the dynamic range.
  ...
  var prefix=_startc.name;
  var formRef=_startc.form;
  formRef.elements[prefix+"_day"].options.selectedIndex=0;
  formRef.elements[prefix+"_mon"].options.selectedIndex=0;
  formRef.elements[prefix+"_year"].options.selectedIndex=0;

  var prefix=_endc.name;
  var formRef=_endc.form;
  formRef.elements[prefix+"_day"].options.selectedIndex=0;
  formRef.elements[prefix+"_mon"].options.selectedIndex=0;
  formRef.elements[prefix+"_year"].options.selectedIndex=0;
