...
var _nn4_calHeight=150;	// the Height of the calendar in Netscape 4.
...
function fPopMulti(p1, p2, p3, p4, p5) {
	...
	for (var n=1; n<=_calendars4sync; n++) {
		ctx=eval("gContainer."+cn[0]+"_"+n);
		if (_first_run) ctx.gPosOffset[1]+=w;
		ctx.fPopCalendar(p1, p2, p3, p4, p5);
		if (_first_run) {w+=NN4?_nn4_calHeight:ctx.fGetById(document,"outerTable").offsetHeight;
		if (n>1&&n<_calendars4sync) w-=2;}
	}
    if (gdSelect[1]>0) {
        var ed = p2?p2:gEnd;
        var offset=ed[1][0]==gdSelect[0]&&ed[1][1]==gdSelect[1]?-1:0;
        for (n=1; n<=_calendars4sync; n++) {
            ctx=eval("gContainer."+cn[0]+"_"+n);
            ctx.fSetCal(gdSelect[0],gdSelect[1]+n-1+offset,0);
        }
    }
    _first_run=false;
}
...
document.onkeydown=function(e){var evt=e?e:event;if(evt.keyCode==27)fHideAll();}
